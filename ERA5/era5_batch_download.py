...
Regions = ['WORLD','ASIA', 'EUAF', 'AFR', 'NAM', 'SAM', 'MLY', 'AUS', 'NZD', ]  # 'EBor','WBor','antarctic','arctic']
...
level_params = ['temperature', 'relative_humidity', 'ozone_mass_mixing_ration', 'specific_humidity', 'u_component_of_wind',
          'v_component_of_wind']  # 'relative_humidity', 'temperature'
single_params = ['2m_temperature', 'surface_pressure', 'forecast_albedo']
    ...
    for im, month in enumerate(Months):
        for id, day in enumerate(Days):
            ...
            if (year not in leapyears) and month == '02' and day >= '29':
                continue
            profile_ncFileName = "C:/Users/RS/Desktop/hwz/" + 'ERA5_profile_' + year + month + day + '.nc'
            if os.path.exists(profile_ncFileName):
                continue
            print(profile_ncFileName + " is downloading")
            c.retrieve('reanalysis-era5-pressure-levels',  # 'reanalysis-era5-pressure-levels',
                       {'product_type': 'reanalysis',
                        'variable': level_params,
                       ...
                       profile_ncFileName)
            surface_ncFileName = "C:/Users/RS/Desktop/hwz/" + 'ERA5_surface_' + year + month + day + '.nc'
            if os.path.exists(surface_ncFileName):
                continue
            print(surface_ncFileName + " is downloading")
            c.retrieve('reanalysis-era5-single-levels',  # 'reanalysis-era5-single-levels',
                       {'product_type': 'reanalysis',
                        'variable': single_params,
                        'year': year,
                        'month': month,
                        'day': day,
                        'time': UTC,

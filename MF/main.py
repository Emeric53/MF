...
def get_subdirectories(folder_path: str) -> (list, list):
    """
    获取指定文件夹中所有子文件夹的路径列表。
...
def main(satellite_name: str):
    if satellite_name == 'GF5B':
        # 设置 输出文件夹 路径
        outputfolder = r"I:\AHSI_test"
        ...
        for filefolder in filefolder_list:
            ...
            for index in range(len(filelist)):
                ...
                if os.path.exists(outputfile):
                    ...
                    ahsi_unit_absorption_spectrum_path = ('C:\\Users\\RS\\PycharmProjects\\Methane'
                                                     '\\unit_absorption_spectrum_ahsi.txt')
                    # 读取单位吸收谱
                    ...
                    ahsibands = np.load('ahsi_bands.npy')[150:]
        ...
        print("Invalid satellite name, please select from 'GF5B' or 'EMIT'.")

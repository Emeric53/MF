...
def get_index(model_type: int, parameter: str):
    if parameter == "alt":
        start_index = 43
...
def get_profile(model_type: int, parameter: str):
    data_list = []
    with open(
        "C:\\Users\\RS\\VSCode\\matchedfiltermethod\\src\\data\\modtran_related\\mlatmb.f",
        "r",
    ) as file:
...
# 计算干空气和甲烷柱浓度
air_column_concentration = trapezoidal_integration(
    altitude_profile, dryair_molar_profile
)
methane_column_concentration = trapezoidal_integration(
    altitude_profile, methane_molar_profile
)

...
def ml_matched_filter(
    ...
    if dynamic_adjust:
        ...
        threshold = 1000
        # 更新背景像素浓度
        bg_conc_mask = concentration <= 1000
        if np.any(bg_conc_mask):
            current_background_spectrum = background_spectrum * bg_uas
            current_target_spectrum = (
                current_background_spectrum * bg_uas
            )  # Example dynamic selection
            radiancediff_with_background = (
                data_cube - current_background_spectrum[:, None, None]
            )
            # Recompute concentration for bg-concentration pixels
            masked_radiancediff = radiancediff_with_background[:, bg_conc_mask]
            numerator = np.einsum(
                "ij,i->j",
                masked_radiancediff,
            ...
            new_concentration = np.maximum(
                numerator / (albedo[bg_conc_mask] * common_denominator), 0
            )

            # Update concentration and threshold
            concentration[bg_conc_mask] = new_concentration
        # 分层调整像素浓度
        i = 1
        ...
        while np.any(high_conc_mask) and threshold < max_threshold:
            current_background_spectrum = (
                background_spectrum * transmittance_list[i - 1]
            )
            current_target_spectrum = (
            ...
            masked_radiancediff = radiancediff_with_background[:, high_conc_mask]
            numerator = np.einsum(
                "ij,i->j",
                masked_radiancediff,
                np.dot(covariance_inverse, current_target_spectrum),
            ...
            new_concentration = (
                numerator / (albedo[high_conc_mask] * common_denominator) + threshold
            )

            # Update concentration and threshold
            concentration[high_conc_mask] = new_concentration
            if threshold < 5000:
                threshold += 2000
            else:
...
def generate_uas_transmittance_list(satelitetype, sza, altitude):
    ...
    _, uas = glut.generate_satellite_uas_for_specific_range_from_lut(
        satelitetype, 0, 1000, 2150, 2500, sza, altitude
    )
    uas_list.append(uas)
    transmittance_list = []
    s = 1000
    while s < 50000:
        if s < 5000:
            _, uas = glut.generate_satellite_uas_for_specific_range_from_lut(
                satelitetype, s, s + 2000, 2150, 2500, sza, altitude
            )
            _, transmittance = glut.generate_transmittance_for_specific_range_from_lut(
                satelitetype, 0, s, 2150, 2500, sza, altitude
            )
            uas_list.append(uas)
            ...
            _, uas = glut.generate_satellite_uas_for_specific_range_from_lut(
                satelitetype, s, s + 5000, 2150, 2500, sza, altitude
            )
            _, transmittance = glut.generate_transmittance_for_specific_range_from_lut(
                satelitetype, 0, s, 2150, 2500, sza, altitude
            )
...
def ml_matched_filter_simulated_image_test():
    ...
    _, initial_uas = glut.generate_satellite_uas_for_specific_range_from_lut(
        "AHSI", 0, 10000, 2150, 2500, 25, 0
    )
    uas_list, transmittance_list = generate_uas_transmittance_list("AHSI", 25, 0)
...
def ml_matched_filter_real_image_test():
    filepath = "/home/emeric/Documents/Lifei_essay_data/GF5B_AHSI_W102.8_N32.3_20220424_003345_L10000118222/GF5B_AHSI_W102.8_N32.3_20220424_003345_L10000118222_SW.tif"
    # outputfolder = "/home/emeric/Documents/Lifei_essay_data/GF5B_AHSI_W102.8_N32.3_20220424_003345_L10000118222"
    ...
    for i in range(0, 50000, 5000):
        _, uas = glut.generate_satellite_uas_for_specific_range_from_lut(
            "AHSI", i, i + 5000, 2150, 2500, sza, altitude
        )
        _, transmittance = glut.generate_transmittance_for_specific_range_from_lut(
            "AHSI", 0, i, 2150, 2500, sza, altitude
        )
        uas_list.append(uas)
        transmittance_list.append(transmittance)

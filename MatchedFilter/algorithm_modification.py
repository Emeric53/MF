...
def profile_matched_filter(
    base_array, data_array: np.array, unit_absorption_spectrum: np.array
):
...
def profilelevel_test1():
    ...
    for i in range(2000, 14000, 2000):
        ...
        _, uas = nf.open_unit_absorption_spectrum(
            ahsi_unit_absorption_spectrum_path, 2150, 2500
        )
        # 初始化变量
        base = None
        concentration = 0
        ...
        for enhancement in enhancements:
            ...
            channels_path = (
                r"C:\\Users\\RS\\VSCode\\matchedfiltermethod\\MyData\\AHSI_channels.npz"
            )
            _, radiance = nf.get_simulated_satellite_radiance(
                filepath, channels_path, 2150, 2500
            )
        ...
        if intercept > 0:
            ax[2].plot(
                x_fit,
                y_fit,
                label=f"uas_end_{i}:y = {slope:.2f}x + {np.abs(intercept):.2f}",
            )
        elif intercept < 0:
            ax[2].plot(
                x_fit,
...
def profilelevel_test2():
    ...
    ahsi_unit_absorption_spectrum_path = "C:\\Users\\RS\\VSCode\\matchedfiltermethod\\MyData\\uas\\AHSI_UAS_end_30000.txt"
    _, base_uas = nf.open_unit_absorption_spectrum(
        ahsi_unit_absorption_spectrum_path, 2150, 2500
    )
    ...
    # 设置base radiance
    filepath = "C:\\PcModWin5\\Bin\\batch\\AHSI_Methane_0_ppmm_tape7.txt"
    channels_path = (
        r"C:\\Users\\RS\\VSCode\\matchedfiltermethod\\MyData\\AHSI_channels.npz"
    )
    ...
    for enhancement in enhancements:
        # 读取模拟辐射数据
        filepath = (
            f"C:\\PcModWin5\\Bin\\batch\\AHSI_Methane_{enhancement}_ppmm_tape7.txt"
        )
    ...
    if intercept > 0:
        ax[2].plot(
            x_fit,
            y_fit,
            label=f"multi-level:y = {slope:.2f}x + {np.abs(intercept):.2f}",
        )
    elif intercept < 0:
        ax[2].plot(
            x_fit,
    ...
    if intercept > 0:
        ax[2].plot(
            x_fit,
            y_fit,
            label=f"single-level:y = {slope:.2f}x + {np.abs(intercept):.2f}",
        )
    elif intercept < 0:
        ax[2].plot(
            x_fit,
...
def modified_matched_filter_with_fixed_bg(
    base_array, data_array: np.array, unit_absorption_spectrum: np.array
):
    # 获取数据 波段 行数 列数 信息， 初始化浓度矩阵
    bands, rows, cols = data_array.shape
...
def imagelevel_test1_1():
    ...
    _, base_radiance = nf.get_simulated_satellite_radiance(
        basefilepath, channels_path, 2150, 2500
    )
    ...
    for enhancement in enhancements:
        ...
        for i in range(len(bands)):
            ...
            noise = np.random.normal(
                0, current / 1000, (row_num, col_num)
            )  # 生成高斯噪声
            simulated_noisyimage[i, :, :] = (
                np.ones((row_num, col_num)) * current + noise
            )  # 添加噪声到原始数据
        ...
        concentration = matched_filter_with_fixed_bg(
            base_radiance, simulated_noisyimage, uas
        )
        concentrationlist.append(np.mean(concentration))
        # visulization
        ax.plot(
            enhancement * np.ones(10000),
...
def imagelevel_test1_2():
    ...
    for enhancement in enhancements:
        ...
        for i in range(len(bands)):
            current = radiance[i]
            noise = np.random.normal(
                0, current / 100, (row_num, col_num)
            )  # 生成高斯噪声
            simulated_noisyimage[i, :, :] = (
                np.ones((row_num, col_num)) * current + noise
            )  # 添加噪声到原始数据

        concentration = modified_matched_filter_with_fixed_bg(
            base_radiance, simulated_noisyimage, uas
        )
        # visulization
        ax.plot(
...
def image_simulation(
    plume, lower_wavelength, upper_wavelength, row_num, col_num, noise_level
):
    # Load the simulated emit radiance spectrum
    total_bands, lut = rl.load_lookup_table(
        "C:\\Users\\RS\\VSCode\matchedfiltermethod\\MyData\\enhanced_radiance\\AHSI_rad_lookup_table.npz"
    )

    bands, unenhanced_radiance = rl.lookup_spectrum(
        0, total_bands, lut, lower_wavelength, upper_wavelength
    )
    ...
    for i in range(row_num):
        for j in range(col_num):
            if plume[i, j] > 0:
                _, enhanced_radiance = rl.lookup_spectrum(
                    plume[i, j], total_bands, lut, lower_wavelength, upper_wavelength
                )
...
def imagelevel_test2_1():
    ...
    for enhancement in enhancements:
        simulated_image, enhanced_mask, unenhanced_mask = twoperc_enhancement(
            enhancement
        )
        # 使用匹配滤波算法计算浓度增强
...
def imagelevel_test2_2():
    ...
    for enhancement in enhancements:
        ...
        # 使用匹配滤波算法计算浓度增强
        result = modified_matched_filter_with_fixed_bg(
            base_radiance, simulated_image, uas
        )
        enhanced = result[enhanced_mask]
        unenhanced = result[unenhanced_mask]
        resultlist.append(np.mean(enhanced))
...
def imagelevel_test2_3():
    ...
    for i in range(30000, 52000, 4000):
        ...
        for enhancement in enhancements:
            ...
            result = mf.matched_filter(
                simulated_image, uas, iterate=False, albedoadjust=False, sparsity=False
            )
            enhanced = result[enhanced_mask]
            unenhanced = result[unenhanced_mask]
            resultlist.append(np.mean(enhanced))
...
def imagelevel_test3_1():
    ...
    for idx, i in enumerate(endpoints):
        ...
        enhancement = mf.matched_filter(
            simulated_image, uas, iterate=True, albedoadjust=True, sparsity=False
        )
        ...
        # 可视化
        ax[0].scatter(
            plume[total_mask], enhancement[total_mask], c=colors[idx], alpha=0.5
        )
        polyfit_plot(
...
def imagelevel_test3_3():
    ...
    for name in names:
        ...
        plume = np.load(plume_path)
        sf = 1
        simulated_image = image_simulation(
            radiance_path, plume, sf, 2150, 2500, 100, 100, 0.01
        )
        ...
        enhancement, _ = mf.matched_filter(
            simulated_image,
            uas,
            is_iterate=False,
            is_albedo=False,

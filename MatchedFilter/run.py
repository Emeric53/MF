...
import MyFunctions.needed_functions as nf
...
def runinbatch(satellite_name: str):
    if satellite_name == "AHSI":
            ...
            for index in range(len(filelist)):
                ...
                if not os.path.exists(filepath):
                    ...
                    cal_file = os.path.join(
                        filelist[index], "GF5B_AHSI_RadCal_SWIR.raw"
                    )
                    ...
                    # 读取单位吸收谱,按照波长范围进行筛选
                    all_uas = nf.open_unit_absorption_spectrum(
                        ahsi_unit_absorption_spectrum_path
                    )
        ...
        emit_uas = (
            "C:\\Users\\RS\\PycharmProjects\\Methane\\unit_absorption_spectrum_emit.txt"
        )
        # 读取单位吸收谱
        ...
        radiance_path = (
            r"I:\EMIT\Radiation_data\EMIT_L1B_RAD_001_20220810T065132_2222205_041.nc"
        )
        # 读取radiance数据
        radiance = ed.get_emit_array(radiance_path)
        # define the path of the unit absorption spectrum file
        ...
        # call the main function to process the radiance file
        enhancement = nf.matched_filter(
            used_radiance, used_uas, is_iterate=False, is_albedo=False, is_filter=False
        )
...
def rumfor_AHSI(filepath, outputfolder, mf_type):
    ...
    if os.path.exists(outputfile):
        ...
        # 读取单位吸收谱
        ahsi_unit_absorption_spectrum_path = r"C:\\Users\\RS\\VSCode\\matchedfiltermethod\\Needed_data\\AHSI_unit_absorption_spectrum.txt"
        bands, uas = nf.open_unit_absorption_spectrum(
        ...
        interval_uas = nf.open_unit_absorption_spectrum(
            ahsi_interval_unit_absorption_spectrum_path, 2100, 2500
        )
        ...
        if mf_type == 0:
            ...
            enhancement = nf.modified_matched_filter(
                radiance,
                uas,
                interval_uas,
                is_iterate=False,
...
def rumfor_EMIT(filepath, outputfolder, mf_type):
    ...
    if os.path.exists(outputfile):
        ...
        # define the path of the unit absorption spectrum file
        EMIT_unit_absorption_spectrum_path = r"C:\Users\RS\VSCode\matchedfiltermethod\MyData\EMIT_unit_absorption_spectrum.txt"
        bands, uas = nf.open_unit_absorption_spectrum(
            EMIT_unit_absorption_spectrum_path, 2100, 2500
        )
        EMIT_interval_unit_absorption_spectrum_path = r"C:\Users\RS\VSCode\matchedfiltermethod\MyData\AHSI_unit_absorption_spectrum.txt"
        bands, interval_uas = nf.open_unit_absorption_spectrum(
            EMIT_interval_unit_absorption_spectrum_path, 2100, 2500
        )

        if mf_type == 0:
            # call the main function to process the radiance file
            enhancement = mf.matched_filter(
                radiance, uas, iterate=False, albedoadjust=False, sparsity=False
            )
        elif mf_type == 1:

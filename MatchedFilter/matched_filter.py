...
from tools.needed_function import read_tiff,export2tiff
...
def modified_matched_filter(data_array: np.array, base_unit_absorption_spectrum: np.array, interval_unit_absorption_spectrum: np.array,
                            is_iterate=False,is_albedo=False, is_filter=False, is_columnwise=False) -> np.array:
    """
    Calculate the methane enhancement of the image data based on the original matched filter
    and the unit absorption spectrum.

    :param data_array: 卫星遥感影像的numpy数组数据
    :param unit_absorption_spectrum: 单位吸收光谱的numpy数组数据
    :param is_iterate: 是否进行迭代运算
    :param is_albedo: 是否添加反照率校正因子
    :param is_filter: 是否添加L1正则化滤波
    :return: numpy array of methane enhancement result
    """
    # 获取 以 波段 行数 列数 为顺序的数据
    bands, rows, cols = data_array.shape
    ...
    if is_columnwise:
        for col_index in range(cols):
          ...
          # 对于非空列，取均值作为背景光谱，再乘以单位吸收光谱，得到目标光谱
            ...
            covariance = np.zeros((bands, bands))
            for i in range(count_not_nan):
                covariance += np.outer(adjusted_column[:, i], adjusted_column[:, i])
            covariance = covariance/count_not_nan
            covariance_inverse = np.linalg.inv(covariance)
            ...
            if np.any(high_concentration_mask):
                # 使用新的单位吸收谱重新计算目标光谱
                background_spectrum = np.nanmean(current_column[:,valid_rows] - 5000*target_spectrum[:, np.newaxis], axis=1)
                ...
                up = (adjusted_column[:, high_concentration_mask].T @ covariance_inverse @ target_spectrum)
            ...
            for iter_num in range(2):
                ...
                # 基于新的目标谱 和 背景光谱 计算协方差矩阵
                adjusted_column = current_column[:, valid_rows] -(albedo[valid_rows, col_index] *concentration[valid_rows, col_index])[None,:]*target_spectrum[:,None] - background_spectrum[:,None]
                covariance = np.zeros((bands, bands))
                for i in range(valid_rows.shape[0]):
                    covariance += np.outer(adjusted_column[:, i], adjusted_column[:, i])
                covariance = covariance/count_not_nan
                ...
                if np.any(high_concentration_mask):
                    # 使用新的单位吸收谱重新计算目标光谱
                    background_spectrum = background_spectrum - 5000*target_spectrum
                    target_spectrum = np.multiply(background_spectrum, interval_unit_absorption_spectrum)
                    adjusted_column[:, high_concentration_mask] = current_column[:, valid_rows][:, high_concentration_mask] - (
                        target_spectrum[:, None] * albedo[valid_rows, col_index][high_concentration_mask].T*5000)-background_spectrum[:, None]
                    ...
                    covariance_inverse = np.linalg.inv(covariance)
                    # 基于新的目标光谱重新计算高浓度像素的甲烷浓度增强值
                    up = (adjusted_column[:, high_concentration_mask].T @ covariance_inverse @ target_spectrum) - l1filter[valid_rows, col_index][high_concentration_mask]
                    down = albedo[valid_rows, col_index][high_concentration_mask] * (target_spectrum.T @ covariance_inverse @ target_spectrum)
                    # 直接更新原数组
                    valid_indices = np.where(valid_rows)[0]
                    high_concentration_indices = valid_indices[high_concentration_mask]
                    concentration[high_concentration_indices, col_index] = np.maximum(up / down + 5000, concentration[high_concentration_indices, col_index])
                        
    if not is_columnwise:
            count_not_nan = np.count_nonzero(data_array)
            ...
            for i in range(rows):
                for j in range(cols):
                    covariance += np.cov(radiancediff_with_back[i,j], rowvar=False)
            covariance = covariance/count_not_nan
            covariance_inverse = np.linalg.inv(covariance)
            for row_index in range(rows):
                for col_index in range(cols):
                    ...
                    # 基于最优化公式计算每个像素的甲烷浓度增强值
                    up = (data_array[:,row_index,col_index].T @ covariance_inverse @ target_spectrum)
                    down = albedo[row_index, col_index] * (target_spectrum.T @ covariance_inverse @ target_spectrum)
                    concentration[row_index, col_index] = up / down
            ...
            if is_iterate:
                ...
                for iter_num in range(5):
                    ...
                    # 更新背景光谱和目标光谱
                    iter_data = data_array - (
                            target_spectrum[:, None,None] * albedo[None,:,:].T *
                            concentration[None,:,:].T
                    )
                    # 计算更新后的背景光谱 和 目标谱
                    background_spectrum = np.nanmean(iter_data, axis=(1,2))
                    target_spectrum = np.multiply(background_spectrum, base_unit_absorption_spectrum)
...
if __name__ == '__main__':
    ...
    # # 设置 输出文件夹 路径
    MF_outputfolder = r"I:\\mf"
    MMF_outputfolder = r"I:\\mmf"
    ...
    if os.path.exists(MF_outputfile) and os.path.exists(MMF_outputfile):
        ...
        if not os.path.exists(MMF_outputfile):
            mmf_enhancement = modifiedmatched_filter(used_radiance, used_uas,used_interval_uas,is_iterate=False,

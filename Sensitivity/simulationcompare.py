...
filtered_data = df[(df['wavelength（nm）'] >= 2000) & (df['wavelength（nm）'] <= 2500)]
...
window_size = 2  # 调整窗口大小以控制平滑程度
new_emit['平滑透射率'] = new_emit['value'].rolling(window=window_size, min_periods=1).mean()


# 绘制图表
plt.figure(figsize=(10, 6))
plt.plot(image['wl'], image['value'], color='red',label=u"真实光谱" )
plt.legend()
plt.plot(new_emit['wl'], new_emit['平滑透射率'], color='blue' ,label=u"模拟光谱")

plt.rcParams['font.sans-serif'] = ['SimHei']
plt.xlabel('Wavelength(nm)')
plt.ylabel('Radiance(uW/sr cm-2 nm)')
plt.title('EMIT-模拟值对比')
